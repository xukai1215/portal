...
var PrintDialog = function(editorUi,title)
{
    ...
    if(title===undefined){
        var d =  $('#myIfm').prop('contentWindow').document;
        var svg = d.getElementsByTagName("svg")[0].innerHTML;

        var gb = graph.getGraphBounds();
        var w = Math.ceil(gb.width*scale);
        var h = Math.ceil(gb.height*scale);
        $.ajax({
			url:"http://222.192.7.75/GeoModelingNew/ExportImgServlet",
			type:"post",
			...
			success:function (result) {
                ...
                var data = "data:image/svg+xml,"+result;
                var img = new Image();
        ...
        var okBtn = mxUtils.button(mxResources.get('ok'), function()
        {
            if($("#model_scene_name").val()===""){
                ...
                $.ajax({
                    url:"http://localhost:8081/GeoModelingNew/ExportImgServlet",
                    type:"post",
                    data:{"w":w,"h":h,"svg":svg},
                    success:function (result) {
                        console.log(w);
                        ...
                        let data = "data:image/svg+xml,"+result;
                        let img = new Image();

                        img.onload = function() {
                            ctx.drawImage(img, 0, 0);
                            let png = canvas.toDataURL();
                            let uid=$(".geDiagramContainer").attr("uid");
                            if((uid!="")&&(uid!=undefined)){
                                $.ajax({
                                    url:"http://localhost:8081/GeoModelingNew/ConceptualModelEditorEditServlet",
                                    type:"post",
                                    data:{"uid":uid,"name":$("#model_scene_name").val(),"conceptual":xml.outerHTML,"png":png},
                                    success:function (result) {
                                        alert(result);
                                    }
                                });
                                ...
                                $.ajax({
                                    ...
                                    type:"post",
                                    data:{"name":$("#model_scene_name").val(),"conceptual":xml.outerHTML,"png":png},
                                    success:function (result) {
                                        alert(result);
                                    }
                                });
 ...
 * Constructs a new print dialog.
 */
PrintDialog.prototype.create = function(editorUi,title)
{
	...
	onePageCheckBox.setAttribute("checked","checked");
