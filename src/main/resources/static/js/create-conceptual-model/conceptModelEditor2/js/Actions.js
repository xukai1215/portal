...
function save_model_decription(graph)
{	
...
Actions.prototype.init = function()
{
	...
	this.addAction('Add_GeoIcon', function() { 
		/*ui.sidebar.addCustomImage("/Services/GetGeoIconImage.aspx?IconUid=cba920c3-bbc5-44d2-bcb3-505184ea34d4",8);*/
        var dlg = new AddIconDialog(ui);
        ui.showDialog(dlg.container, 700, 525, true, false);
        dlg.init();
		//! TODO
		//! 打开一个模态框，从分类体系中选中一个Icon，加载到左侧边栏（从0-9，依次是不同的分类）
	});
	
	this.addAction('Open_Task', function()
	{
		/*window.openNew = true;
		window.openKey = 'open';
		ui.openFile();*/
		//! TODO
		//ui.sidebar.loadIconFromTask(null,true);
        var dlg = new OpenTaskDialog(ui);
        ui.showDialog(dlg.container, 600, 480, true, false);
        dlg.init();
	});
	
	this.addAction('Save_Task', function() 
	{ 
		//! TODO
        var dlg = new ConfirmSaveTaskDialog(ui);
        ui.showDialog(dlg.container, 220, 100, true, false);
		
	}, null, null, 'Ctrl+S').isEnabled = isGraphEnabled;
	
	this.addAction('Load_Model_Scene', function()
	{
		//! TODO
		//! 打开模态框，打开文件，加载到当前Graph
        ...
        if(uid.length===0){
            var dlg = new LoadConceptualModelDialog(ui);
            ui.showDialog(dlg.container, 620, 480, true, false);
            dlg.init();
		}

	}).isEnabled = isGraphEnabled;
	
	this.addAction('Publish_Model_Scene', function() 
	{
		/*save_model_decription(graph);
		var xml = ui.editor.getGraphXml();
		alert(xml.outerHTML);*/
        ui.showDialog(new PrintDialog(ui,"save").container, 330, 80, true, true);
		//! TODO
		
	}, null, null, 'Ctrl+Shift+S').isEnabled = isGraphEnabled;
	
	this.addAction('Export_Image', function() 
	{
        $("#myIfm").html("");
        ui.showDialog(new PrintDialog(ui).container, 0, 0, true, true);
        ui.hideDialog();

    });

	this.addAction('editModel...', function(callback)
	{
		var cell = graph.getSelectionCell() || graph.getModel().getRoot();
		
		if (cell != null)
		{
			var dlg = new EditRelatedModel(ui, cell,callback);
			ui.showDialog(dlg.container, 600, 500, true, false);
			dlg.init();
		}
	});
	
	this.addAction('editConcept...', function(callback)
	{
		var cell = graph.getSelectionCell() || graph.getModel().getRoot();
		
		if (cell != null)
		{
			var dlg = new EditRelatedConcept(ui, cell, callback);
			ui.showDialog(dlg.container, 600, 500, true, false);
			dlg.init();
		}
	});
